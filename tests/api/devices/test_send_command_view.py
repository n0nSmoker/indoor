...
@pytest.mark.parametrize("command,device_ids", [
    (DEVICE.COMMANDS[0][1], [f'{key1}'],),
    (DEVICE.COMMANDS[1][1], [f'{key1},{key2}'],),
    (DEVICE.COMMANDS[1][1], [f'{key1},{key2},{key3}'],),
])
def test_default(client, command, device_ids):
    ...
    assert command.encode('ascii') in app.cache.storage.lrange(redis_key + device_ids[0], start=0, end=-1)

    # Deleting a test records in redis storage
    for i in device_ids:
        assert command.encode('ascii') == app.cache.storage.rpop(redis_key + i)
...
@pytest.mark.parametrize("command,device_ids", [
    ...
    ('', [f'{key1}'],),
    ('test', [f'{key1}'],),
    (get_random_str(), [f'{key1}'],),

    # Malformed device_ids
    (DEVICE.COMMANDS[0][1], None,),
    (DEVICE.COMMANDS[0][1], [''],),
