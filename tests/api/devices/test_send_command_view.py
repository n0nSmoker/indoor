...
@pytest.mark.parametrize("command,device_ids,redis_key", [
    ('info', [1], 'test'),
    # ('restart_device', [1, 2, 3],),
    # ('logs', [1, 2, 3, 4],),
])
def test_default(client, app, command, device_ids, redis_key):
    resp = client.post(
        endpoint=endpoint,
        data=dict(
            command=command,
...
def test_malformed_params_failure(client, app, command, device_ids, redis_key):
    resp = client.post(
        endpoint=endpoint,
        data=dict(
            command=command,
